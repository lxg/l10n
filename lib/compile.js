...
// if sources are provided, we are going to only add them. Otherwise compile entire catalogs.

export default async function(catalogs, files, extras) {
    const table = {}
        ...
        Object.keys(catalogs[locale].translations).forEach(context => {
            Object.keys(catalogs[locale].translations[context]).forEach(msgid => {
                if (msgid) {
                    ...
                    const isContained = !files || (entry.locations && entry.locations.filter(location => files.includes(location.file)).length);
